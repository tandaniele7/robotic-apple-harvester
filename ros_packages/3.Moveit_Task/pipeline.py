...
from utils import failed, success, create_collision_sphere
...
class ApplePickingPipeline(Node):
    def __init__(self):
        ...
        # Clients
        self.update_scene_client = self.create_client(
            Trigger, "update_scene", callback_group=self.callback_group
        )
        self.get_apple_pose_client = self.create_client(
            GetApplePose, "get_apple_pose", callback_group=self.callback_group
        )
        self.starting_position_client = self.create_client(
            Trigger, "start_capturing", callback_group=self.callback_group
        )
        self.move_manipulator_client = self.create_client(
            MoveManipulator, "move_manipulator", callback_group=self.callback_group
        )
        self.place_apple_client = self.create_client(
            Trigger, "apple_to_box", callback_group=self.callback_group
        )
        ...
        self.gripper_command_client = ActionClient(
            self,
            GripperCommand,
            "/gen3_lite_2f_controller/gripper_cmd",
            callback_group=self.callback_group,
        )
        self.collision_apple_publisher = self.create_publisher(
        ...
        self.pipeline_timer = self.create_timer(
            0.5, self.run_pipeline_step, callback_group=self.callback_group
        )
    ...
    def run_pipeline_step(self):
        with self.lock:
            ...
            self.get_logger().info(f"Pipeline state: {self.pipeline_state}")

            if self.pipeline_state == "SETUP":
                self.setup_time = self.get_time()
                ...
                self.get_apple_pose_time = self.get_time()
    ...
    def update_scene(self):
        ...
        if not self.update_scene_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "/update_scene service not available, waiting again..."
            )
    ...
    def update_scene_callback(self, call_future, future):
        try:
            ...
            if result.success:
                self.inference_time = self.get_time() - self.init_time
    ...
    def get_apple_pose(self):
        ...
        if not self.get_apple_pose_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().info(
                "get_apple_pose service not available, waiting again..."
            )
            future.set_result(None)
            return future
    ...
    def get_apple_pose_callback(self, call_future, future):
        try:
            ...
            if result.success:
                ...
                self.radius = result.radius.data
                # list of coordinates, different from above (Pose message)
                self.appl_pt = [self.apple_pose.pose.position.x, self.apple_pose.pose.position.y, self.apple_pose.pose.position.z]
                self.collision_apple_publisher.publish(create_collision_sphere(self.appl_pt, self.radius, "add"))
    ...
    def move_manipulator(self):
        ...
        if not self.move_manipulator_client.wait_for_service(timeout_sec=5.0):
            self.get_logger().info(
                "/move_manipulator service not available, waiting again..."
            )
            future.set_result(None)
            return future
        ...
        call_future.add_done_callback(
            lambda f: self.move_manipulator_callback(f, future)
        )
        return future
    ...
    def move_manipulator_callback(self, call_future, future):
        try:
            result = call_future.result()
            self.collision_apple_publisher.publish(create_collision_sphere(self.appl_pt, self.radius, "remove"))

            
            if result.success:
                self.pipeline_state = "GRASP"
            else:
                self.test_data = failed(
                    self.get_apple_pose_time,
                    self.get_time(),
                    self.test_data,
                    f"Failed to move manipulator to Target Position: {result.message}",
                    atmp=self.attempt,
                    appl_ps=self.apple_pose
                )
    ...
    def starting_position(self):
        ...
        if not self.starting_position_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn(
                "Impossible to move to starting position: service not available, waiting again..."
            )
            future.set_result(None)
            return future
        ...
        call_future.add_done_callback(
            lambda f: self.starting_position_callback(f, future)
        )
        return future
    ...
    def gripper_command(self, position):
        ...
        if position == "close":
            goal_msg.command.position = 0.41
        ...
        send_goal_future.add_done_callback(
            lambda f: self.gripper_command_response_callback(f, future)
        )
        return future
    ...
    def gripper_command_response_callback(self, send_goal_future, future):
        ...
        get_result_future.add_done_callback(
            lambda f: self.gripper_command_result_callback(f, future)
        )
        ...
        try:
            ...
            if result.success:
                self.test_data = success(
                    ...
                    motion_time= self.get_time() - self.get_apple_pose_time,
                    test_dt=self.test_data,
                    atmp_=self.attempt,
                    appl_ps_=self.apple_pose
                )
                self.pipeline_state = "SETUP"
            else:
                # it keeps trying to go to starting position
