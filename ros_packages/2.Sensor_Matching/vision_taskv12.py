...
class Detection(Node):
                    ...
                    if apple_points:  # Check if the apple_points list is not empty
                        ...
                        if len(apple_points_np) > 0:
                            ...
                            radius, distance_to_camera = estimate_apple_radius(
                                apple_points_np, self.depth_intrin
                            )
                            ...
                            self.get_logger().info(
                                f"Apple {apple_index} - Center: {center}, Estimated Radius: {radius}, Distance to Camera: {distance_to_camera}"
                            )
    ...
    def get_apple_pose_callback(self, request, response):
        if self.apple_ids:
            ...
            shifted_apple_pt = PointStamped()

            shifted_apple_pt = shift_point(
                response.pose.pose.orientation,
                Vector3(x=x, y=y, z=z),
                0.04,
            )

            response.pose.pose.position.x = shifted_apple_pt[0]
            response.pose.pose.position.y = shifted_apple_pt[1]
            response.pose.pose.position.z = shifted_apple_pt[2]

            self.broadcast_transform(apple_pose=shifted_apple_pt)
