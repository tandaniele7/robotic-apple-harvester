...
from masks_detection import detect_masks, display_masks
...
class Detection(Node):
    ...
    def update_scene_callback(self, request, response):
        ...
        if (
            ...
            #cv2.imwrite("color.png", self.image_rgb)
            apple_mask, branches_mask, metal_wire_mask, background_mask = detect_masks(
                ...
                conf_apples=0.72,
                conf_branches=0.6,
                conf_metal_wire=0.68,
                conf_background=0.8,
            )
            
            apple_points, branches_points, wire_points, leaves_points = (
                self.depth_to_xyz(
                    self.depth,
                    self.depth_intrin,
                    apple_mask,
            ...
            # Leaves are not considered since the will probably occupy the entire planning scene
    ...
    def get_apple_pose_callback(self, request, response):
        ...
        if self.apple_ids != []:
            ...
            print(self.apple_ids)
            response.pose.header.frame_id = self.frame_id
            response.pose.pose.position.x = self.apple_ids[0]["center"]["x"]
            response.pose.pose.position.y = self.apple_ids[0]["center"]["y"]
            response.pose.pose.position.z = self.apple_ids[0]["center"]["z"]

            # response.pose.pose.orientation.x = self.apple_ids[0]["center"]["z"]
            # response.pose.pose.orientation.y = self.apple_ids[0]["center"]["z"]
            # response.pose.pose.orientation.z = self.apple_ids[0]["center"]["z"]
            # response.pose.pose.orientation.w = self.apple_ids[0]["center"]["z"]
