...
class Detection(Node):
    ...
    def spin_callback(self):
        ...
        if (np.array(self.image_rgb).shape != ()
            ...
            display_masks(apple=apple_mask, branches=branches_mask, background= background_mask, metal_wire= metal_wire_mask)

            for point, color in zip(np.asarray(self.pcd.points), np.asarray(self.pcd.colors)):
                x, y, z = point
                r, g, b = (color * 255).astype(np.uint8)
                pt = rs.rs2_project_point_to_pixel(self.depth_intrin, point)
                print(pt)
                u = int(fx * x / z + cx)
                v = int(fy * y / z + cy)
