...
def display_masks(apple, branches, metal_wire, background):
...
def detect_masks(
    ...
    res = m.predict(image_obj, show=show_opt, save = save_opt)
    ...
    apple_ms = np.zeros((H, W)).astype("uint8")
    branches_ms = np.zeros((H, W)).astype("uint8")
    metal_wire_ms = np.zeros((H, W)).astype("uint8")
    background_ms = np.zeros((H, W)).astype("uint8")
        ...
        for ci, c in enumerate(r):
            ...
            try:
                ...
                if label == "apple" and confidence >= conf_apples:
                    apple_ms = np.maximum(apple_ms, mask)
    ...
    return apple_ms, branches_ms, metal_wire_ms, background_ms



if __name__ == "__main__":

    apple_masks, background_masks, metal_wire_masks, branches_masks = detect_masks(
        show_opt=False
    )
    display_masks(
        apple=apple_masks,
