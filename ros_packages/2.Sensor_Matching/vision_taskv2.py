        ...
        if (
            ...
            if list(apple_points) != []:
                ...
                if len(apple_points_np) > 0:
                    # Normalize the data to prevent very large values
                    apple_points_scaled = (apple_points_np - np.mean(apple_points_np, axis=0)) / np.std(apple_points_np, axis=0)
                    
                    try:
                        clustering = DBSCAN(eps=0.05, min_samples=7).fit(apple_points_np)
                    ...
                    self.get_logger().warn("No valid apple points found after filtering")
